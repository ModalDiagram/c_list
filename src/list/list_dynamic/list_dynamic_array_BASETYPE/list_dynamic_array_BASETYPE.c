...
#ifndef GET_PNEXT
#define GET_PNEXT(pvalue) (*((ppvoid)(((pchar) pvalue )+sizeof_array)))
#endif
...
pvoid malloc_list_dynamic_array_BASETYPE(unsi dim_array){
  ...
  pnew_list->n_elem = 0;
  pnew_list->sizeof_array = dim_array * sizeof(BASETYPE_SPACE);
  return pnew_list;
 }
...
unsi get_n_elem_dynamic_array_BASETYPE(pvoid plist){
  return ((plist_dynamic_array_BASETYPE) plist)->n_elem;
 }
